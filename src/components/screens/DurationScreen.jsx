...
export default function DurationScreen({ onNext, onBack, formData, setFormData }) {
  ...
  return (
    <div className="flex flex-col items-center w-full h-full relative" style={{ padding: '16px' }}>
      ...
      {/* Navigation */}
      <motion.div className="flex gap-3 mt-4 w-full max-w-2xl" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }}>
        <motion.button
        ...
        <motion.button
          ...
          style={{
            ...
            border: '1px solid rgba(255, 215, 0, 0.4)', color: '#FFD700', cursor: 'pointer',
          }}
          whileHover={{ boxShadow: '0 0 15px rgba(255, 215, 0, 0.3)' }}
          whileTap={{ scale: 0.97 }}
          onClick={handleUndo}
        >↩️ UNDO</motion.button>
        <motion.button
          className="flex-1 py-3 rounded-xl font-bold relative overflow-hidden"
          style={{
            fontFamily: 'Orbitron, sans-serif', fontSize: '13px', letterSpacing: '0.15em',
